...
fd vtd-connected.json dual-graphs -j 1 -x bash -c 'timeout 600 ./target/release/balanced-edges -f {} -d $(python python/seats_lookup.py {} --chamber congress) -t 0.01 > logs/{/}.congress-1.log'
fd vtd-connected.json dual-graphs -j 1 -x bash -c 'timeout 600 ./target/release/balanced-edges -f {} -d $(python python/seats_lookup.py {} --chamber congress) -t 0.03 > logs/{/}.congress-3.log'
